...
export default function Schedule() {
  ...
  const [scheduledRides, setScheduledRides] = useState([]); // State for stored rides
  const [viewMode, setViewMode] = useState("schedule"); // "schedule" or "view"

  // Fetch scheduled rides when the component mounts or user changes
  useEffect(() => {
  ...
  const scheduleRide = async () => {
    try {
      ...
      // Refresh the scheduled rides list after scheduling
      const ridesRef = collection(db, "users", user, "scheduled_rides");
      const ridesSnapshot = await getDocs(ridesRef);
      const ridesList = ridesSnapshot.docs.map((doc) => ({
        id: doc.id,
  ...
  // Render a single scheduled ride item
